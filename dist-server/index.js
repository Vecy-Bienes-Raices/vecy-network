...
function extractPermutaPercentage(text2) {
  const t2 = (text2 || "").toLowerCase();
  const slashMatch = t2.match(/(\d{1,3})\s*\/\s*(\d{1,3})/);
  if (slashMatch) {
    const a = parseInt(slashMatch[1], 10);
    const b = parseInt(slashMatch[2], 10);
    if (a + b === 100) {
      const ventaFirst = t2.indexOf("venta") < t2.indexOf("permuta") || !t2.includes("permuta");
      return ventaFirst ? { ventaPct: a, permutaPct: b } : { ventaPct: b, permutaPct: a };
    }
  }
  const pctMatch = t2.match(/(?:venta|vdo|vdo\.)\s*(\d{1,3})\s*%.*?(?:permuta|carro|vehiculo|bien)\s*(\d{1,3})\s*%/);
  if (pctMatch) {
...
function explicarMatch(requirement, property) {
  ...
  if (budgetMax > 0 && effectivePropPriceForCheck <= 0) {
    blockers.push("Match inviable: La oferta NO especifica precio comercial v\xE1lido (N/E) y el requerimiento exige presupuesto.");
    return buildExplanationResult(0, blockers, positives, negatives);
  }
  ...
  const deduceFullType = (typeRaw, text2) => {
    ...
    if (t2 === "land" || t2 === "lote" || t2 === "terreno" || t2 === "predio") return "land";
    if (t2 === "building" || t2 === "edificio") return "building";
    if (t2 === "hotel" || t2 === "hostal" || t2 === "aparta_hotel" || t2 === "motel" || t2 === "aparta_suit") return "hotel";
    if (t2 === "cabin" || t2 === "caba\xF1a") return "cabin";
    if (t2 === "loft" || t2 === "apartaestudio" || t2 === "apartasuite" || t2 === "aparta_suite") return "loft";
    if (t2 === "villa") return "villa";
    if (t2 === "house" || t2 === "casa" || t2.includes("casa_")) return "house";
    if (t2 === "penthouse" || t2 === "pent_house" || t2 === "ph") return "apartment";
    ...
    if (clean.includes("aparta hotel") || clean.includes("aparta-hotel") || clean.includes("apartahotel") || clean.includes("aparta suit") || clean.includes("aparta-suit") || clean.includes("apartasuit") || clean.includes("motel") || clean.includes("hostal") || clean.includes("hostel")) {
      return "hotel";
    }
    if (/\bhotel\b/.test(clean)) return "hotel";
    if ((clean.includes("casa") || clean.includes("townhouse") || clean.includes("chalet") || clean.includes("house")) && !clean.includes("apartamento") && !clean.includes("apto")) {
      return "house";
    }
    ...
    if (clean.includes("lote") || clean.includes("terreno") || clean.includes("predio") || clean.includes("land") || clean.includes("suelo urbano") || clean.includes("suelo rural") || clean.includes("suelo suburbano") || clean.includes("suelo expansion") || clean.includes("suelo de expansion") || clean.includes("suelo proteccion")) {
      return "land";
    }
    if (clean.includes("apartaestudio") || clean.includes("aparta estudio") || clean.includes("apartasuite") || clean.includes("aparta suite") || clean.includes("loft")) {
      return "loft";
    }
  ...
  if (effectiveReqType && effectivePropType) {
    ...
    const isLoftAptoCrossCompatible = isLoftReq && isAptoProp && (pBedrooms <= 1 || propArea <= 65 || propArea === 0) || isAptoReq && isLoftProp && (reqBedrooms <= 1 || reqAreaMin <= 65 || reqAreaMin === 0);
  ...
  const getHorizontalPropertySubtype = (type, raw) => {
    ...
    if (t2 === "hotel" || r.includes("hotel") || r.includes("hostal") || r.includes("aparta hotel") || r.includes("aparta suit") || r.includes("motel")) {
    ...
    if (t2 === "villa" || /\bvilla\b/.test(r) && !r.includes("villa magdala") && !r.includes("villavicencio")) {
      return "villa";
    }
    if (t2 === "warehouse" || t2 === "bodega" || r.includes("bodega")) return "bodega";
    if (t2 === "commercial" || t2 === "local" || r.includes("local comercial") || /\blocal\b/.test(r)) return "local";
    if (t2 === "office" || r.includes("oficina")) {
    ...
    if (t2 === "land" || r.includes("lote") || r.includes("terreno") || r.includes("predio")) {
    ...
    if (t2 === "farm" || t2 === "finca" || r.includes("finca") || r.includes("casa de campo")) {
      return "finca";
    }
    if (t2 === "cabin" || r.includes("caba\xF1a") || r.includes("cabana")) return "caba\xF1a";
    if (r.includes("apartaestudio") || r.includes("aparta estudio") || r.includes("suite ejecutiva") || r.includes("alcoba independiente") || t2 === "apartaestudio" || t2 === "studio") {
      return "apartaestudio";
    }
    if (r.includes("loft") || t2 === "loft") return "loft";
    const isPH = r.includes("penthouse") || r.includes("pent house") || /\bph\b/.test(r) || t2 === "penthouse";
    const isDuplex = r.includes("duplex") || r.includes("d\xFAplex") || r.includes("triplex") || r.includes("tr\xEDplex");
    ...
    if (t2 === "house" || t2 === "casa" || r.includes("casa")) {
      ...
      if (r.includes("campestre") || r.includes("finca raiz") || r.includes("finca ra\xEDz")) return "casa_campestre";
      if (r.includes("quinta") || r.includes("mansion")) return "casa_quinta";
      if (r.includes("condominio")) return "casa_condominio";
      if (r.includes("conjunto") || r.includes("unidad") || r.includes("urbanizacion") || r.includes("urbanizaci\xF3n")) return "casa_conjunto";
  ...
  const SUBTYPE_COMPATIBILITY_GROUPS = {
  ...
  if (isReqOpenBudget) {
    ...
    if (isReqRent2) {
      ...
      const budgetMin2 = requirement.presupuestoMin ? parseFloat(String(requirement.presupuestoMin)) : 0;
      let salePrice = price;
      if (salePrice <= 0 || salePrice < 3e7) {
        blockers.push(`Guillotina Financiera (Tolerancia Cero): La oferta no especifica un precio de venta comercial v\xE1lido (N/E o menor a $30M) para contrastar con el presupuesto m\xE1ximo de $${budgetMax.toLocaleString()}. Match inviable (0%).`);
        return buildExplanationResult(0, blockers, positives, negatives);
      }
...
var init_nameAndGenderResolver = __esm({
  "server/_core/nameAndGenderResolver.ts"() {
    ...
    VECY_COMMERCIAL_INFO = {
      ...
      services: "Estudios de mercado m\xB2 y canon sugerido \u26A1 | Consultor\xEDa jur\xEDdica y tributaria \u2696\uFE0F | Manejo de cobranzas \u{1F4BC} | Marketing con IA y fotograf\xEDa \u{1F916}\u{1F4F8} | Contratos digitales \u{1F4C4}",
      phone: "3166569719",
      schedule: {
...
__export(janIA_exports, {
  ...
  cleanUnescapedQuotesInJSON: () => cleanUnescapedQuotesInJSON,
...
function isPhoneNumberNotPrice(val, rawText) {
  ...
  if (num >= 5e7 && num % 1e5 === 0) return false;
  if (num >= 3e5 && num <= 5e7 && num % 5e4 === 0) return false;
  const numStr = String(Math.round(num));
  if (numStr.length === 10 && /^3(0[0-5]|1[0-9]|2[0-4]|5[0-1])/.test(numStr) && num % 1e6 !== 0) {
    if (rawText && /(?:\$|precio|valor|ppto|presupuesto|canon|hasta|venta)\s*:?\s*\$?\s*3\d{9}/i.test(rawText)) return false;
    return true;
  }
  if (numStr.length === 12 && numStr.startsWith("573") && num % 1e6 !== 0) {
...
function cleanUnescapedQuotesInJSON(content) {
...
function extractFallbackDataFromText(text2) {
  ...
  if (adminFee === 0) {
    ...
    if (adminMilMatch) {
      ...
      if (!isNaN(numParsed) && numParsed >= 20 && numParsed <= 15e3) {
        ...
        if (calculatedFee >= 5e4 && calculatedFee <= 15e6) {
  ...
  if (canonMatch) {
    ...
    if (computed > 0 && computed <= 1e8 && !isPhoneNumberNotPrice(computed, text2)) {
      rentPrice = computed;
    }
  }
  ...
  if (saleMatch) {
    ...
    if (computed >= 3e7 && !isPhoneNumberNotPrice(computed, text2)) {
      price = computed;
  ...
  if (price === 0 && rentPrice === 0) {
    ...
    if (simplePriceMatch) {
      ...
      if (computed > 0 && !isPhoneNumberNotPrice(computed, text2) && computed !== adminFee) {
        if (transactionType === "arriendo" || computed <= 5e7) {
          rentPrice = computed;
        } else {
          price = computed;
  ...
  let areaMin = 0;
  let areaMax = 0;
  const dimMultMatch = (text2 || "").match(/(\d+(?:[.,]\d+)?)\s*[*xX]\s*(\d+(?:[.,]\d+)?)\s*(?:m2|mts2|mts|mt2|metros(?:\s+cuadrados)?|m²)?/i);
  if (dimMultMatch) {
  ...
  let city = "";
  const isBogotaContext = clean.includes("bogota") || clean.includes("bogot\xE1") || clean.includes("cedritos") || clean.includes("chico") || clean.includes("chic\xF3") || clean.includes("rosales") || clean.includes("usaquen") || clean.includes("usaqu\xE9n") || clean.includes("santa barbara") || clean.includes("santa b\xE1rbara") || clean.includes("chapinero") || clean.includes("suba") || clean.includes("engativa") || clean.includes("engativ\xE1") || clean.includes("tabora") || clean.includes("floresta") || clean.includes("santa maria del lago") || clean.includes("santa mar\xEDa del lago") || clean.includes("fontibon") || clean.includes("fontib\xF3n") || clean.includes("kennedy") || clean.includes("teusaquillo") || clean.includes("salitre") || clean.includes("barrios unidos");
  if (isBogotaContext) {
    city = "Bogot\xE1, D.C.";
...
async function processWhatsAppMessage(text2, userId, userName, hasMedia = false, scrapedData = [], audioUrl, imageBuffer, isGroup = false, pdfBuffer, pdfMimeType, groupJid, groupName, flyerVisionData) {
  try {
    ...
    if (text2 && !text2.includes("__is_sub_message__")) {
      const subBlocks = splitMultiItemMessage(text2);
      if (subBlocks.length >= 2) {
        console.log(`[JanIA-MultiItemSplitter] \u{1F680} Ingestando ${subBlocks.length} publicaciones individuales de manera independiente...`);
...
async function saveProperty(data, userId, realName, imageBuffer, pdfBuffer, pdfMimeType) {
  ...
  if (isVentaType && (currentPriceVal < 1e8 || !data.price) && data.rawText) {
    ...
    } else if (currentPriceVal < 3e7) {
      if ((!data.adminFee || parseFloat(String(data.adminFee)) <= 0) && currentPriceVal >= 1e5 && currentPriceVal <= 3e7) {
...
4. **\u{1F3AF} Estratega de Marketing Digital Inmobiliario y T\xE9cnicas de Venta**:
   ...
   - La Estructura de 7 Pilares de Ofertas y Demandas para captar clientes, acelerar cierres y potenciar el matching inteligente de JanIA.
   - Consejos de fotograf\xEDa y video con smartphone (encuadre horizontal, iluminaci\xF3n natural, planos abiertos, orden visual).
   - Campa\xF1as y anuncios en Meta Ads y Google Ads: segmentaci\xF3n de compradores, llamados a la acci\xF3n y captaci\xF3n de exclusivas.
   - M\xE9todos gratuitos de viralizaci\xF3n en redes sociales y posicionamiento org\xE1nico en Google.
5. **\u{1F4BC} Gestora de Cobranzas y Cartera de Arrendamiento (Cobranza Extrajudicial y Acuerdos de Pago)**:
   - Asesor\xEDa t\xE9cnica en manejo de inquilinos morosos y cobro de c\xE1nones de arriendo atrasados bajo la Ley 820 de 2003.
   - Redacci\xF3n de requerimientos formales de pago, acuerdos conciliatorios de pago en cuotas, actas de restituci\xF3n voluntaria anticipada y cl\xE1usulas penales sin necesidad de iniciar procesos judiciales largos y costosos.
...
function getBogotaTimeInfo(d = /* @__PURE__ */ new Date()) {
  ...
  let period = "ma\xF1ana";
  let greeting = "\xA1Buenos d\xEDas!";
  if (hour >= 5 && hour < 12) {
    period = "ma\xF1ana";
    greeting = "\xA1Buenos d\xEDas!";
  } else if (hour >= 12 && hour < 19) {
    period = "tarde";
    greeting = "\xA1Buenas tardes!";
  } else {
    period = "noche";
    greeting = "\xA1Buenas noches!";
  }
  return { hour, min, period, greeting, timeStr: bogotaTimeStr };
}
function enforceGreetingAccuracy(text2, period) {
  if (!text2) return text2;
  let res = text2;
  if (period === "tarde") {
    res = res.replace(/¡?buenos\s+días!?/gi, "\xA1Buenas tardes!");
    res = res.replace(/buen\s+día/gi, "buena tarde");
  } else if (period === "noche") {
    res = res.replace(/¡?(?:buenos\s+días|buenas\s+tardes)!?/gi, "\xA1Buenas noches!");
    res = res.replace(/(?:buen\s+día|buena\s+tarde)/gi, "buena noche");
  } else if (period === "ma\xF1ana") {
    res = res.replace(/¡?(?:buenas\s+tardes|buenas\s+noches)!?/gi, "\xA1Buenos d\xEDas!");
    res = res.replace(/(?:buena\s+tarde|buena\s+noche)/gi, "buen d\xEDa");
  }
  return res;
}
...
\u{1F4CC} *Titular:* ${opts?.headline || "Din\xE1mica del Mercado Inmobiliario y Proyecciones en Colombia"}

${opts?.details || "El mercado inmobiliario en Colombia contin\xFAa mostrando movimientos estrat\xE9gicos en colocaci\xF3n de cr\xE9ditos, valorizaci\xF3n de metro cuadrado y demanda de arrendamientos urbanos y tur\xEDsticos."}

\u{1F4A1} *Impacto Comercial:* Recuerda que un corredor informado cierra m\xE1s y mejor. Utiliza esta informaci\xF3n para orientar a tus propietarios compradores y arrendatarios con bases s\xF3lidas.

\u{1F4F2} *Consultas con JanIA:* https://vecy-network.vercel.app/jania`;
...
function invalidateRequirementsCache() {
  cachedRequirementsTime = 0;
  cachedRequirementsData = null;
}
...
- ciudadDeseada: Nombre de la ciudad (por defecto "Bogot\xE1" si no especifica)
- addressNeighborhood: Barrio espec\xEDfico o sectores mencionados (ej: "Chic\xF3, Rosales, Virrey")
- zonaDeseada: Sector o zona general si aplica (ej: "Norte", "Chapinero")
- presupuestoMin: valor num\xE9rico string sin puntos ni s\xEDmbolos o null
- presupuestoMax: valor num\xE9rico string sin puntos ni s\xEDmbolos (ej: "450000000" para compra, o "3500000" para arriendo) o null
- areaMin: metros cuadrados m\xEDnimos en n\xFAmero string (ej: "75") o null
- habitacionesMin: n\xFAmero entero m\xEDnimo o null
- banosMin: n\xFAmero entero m\xEDnimo o null
- parqueaderosMin: n\xFAmero entero m\xEDnimo o null
- adminFeeMax: cuota de administraci\xF3n m\xE1xima en n\xFAmero string o null
- estratoDeseado: array de n\xFAmeros enteros (ej: [4, 5]) o null
- amobladoDeseado: boolean o null
- caracteristicasDeseadas: array de strings con comodidades clave (ej: ["balc\xF3n", "ascensor", "dep\xF3sito"])
- rawText: Texto original analizado

Si un campo no est\xE1 expl\xEDcito en el texto, coloca null (NO inventes informaci\xF3n).
...
Texto: ${input.text}`;
      ...
      if (!parsed.addressNeighborhood && !parsed.zonaDeseada) {
        missingFields.push("Barrio o Sector de Inter\xE9s");
      }
      ...
      if (!parsed.areaMin || String(parsed.areaMin).trim() === "" || Number(parsed.areaMin) === 0) {
        missingFields.push("\xC1rea M\xEDnima (m\xB2)");
      }
      if (!parsed.estratoDeseado || Array.isArray(parsed.estratoDeseado) && parsed.estratoDeseado.length === 0) {
        missingFields.push("Estrato Socioecon\xF3mico");
      }
      if (parsed.habitacionesMin === void 0 || parsed.habitacionesMin === null) {
        missingFields.push("M\xEDnimo de Habitaciones");
      }
      if (parsed.banosMin === void 0 || parsed.banosMin === null) {
        missingFields.push("M\xEDnimo de Ba\xF1os");
      }
      if (parsed.parqueaderosMin === void 0 || parsed.parqueaderosMin === null) {
...
- name: T\xEDtulo breve y claro de la b\xFAsqueda (ej: "Apto arriendo Chic\xF3 2 habs")
- tipoInmuebleDeseado: "apartment" | "house" | "building" | "warehouse" | "farm" | "hotel" | "office" | "land" | "commercial" | "loft" | "consultorio"
- tipoNegocioDeseado: "venta" | "arriendo" | "venta_o_arriendo" | "arriendo_temporal" | "arriendo_con_opcion_de_compra" | "permuta" | "venta_permuta" | "aporte"
- ciudadDeseada: Nombre de la ciudad (por defecto "Bogot\xE1" si no se especifica)
- addressNeighborhood: Barrio espec\xEDfico o sectores mencionados
- zonaDeseada: Sector o zona general
- presupuestoMin: valor num\xE9rico string sin puntos ni s\xEDmbolos o null
- presupuestoMax: valor num\xE9rico string sin puntos ni s\xEDmbolos (ej: "450000000" para compra, o "3500000" para arriendo) o null
- areaMin: metros cuadrados m\xEDnimos en n\xFAmero string (ej: "80") o null
- habitacionesMin: n\xFAmero entero m\xEDnimo o null
- banosMin: n\xFAmero entero m\xEDnimo o null
- parqueaderosMin: n\xFAmero entero m\xEDnimo o null
- adminFeeMax: cuota de administraci\xF3n m\xE1xima en n\xFAmero string o null
...
- idUsuarioWhatsapp: Tel\xE9fono o WhatsApp de contacto si aparece en el flyer (solo n\xFAmeros)

Si un dato no est\xE1 en el flyer, coloca null (NO inventes informaci\xF3n).
Devuelve \xDANICAMENTE el objeto JSON sin bloques de c\xF3digo ni comentarios.`;
      const response = await invokeLLM({
  ...
  createRequirement: publicProcedure.input(z2.object({
    ...
    const inserted = await db.insert(requirements).values({
      ...
      origenNombre: "Centro de Publicaci\xF3n Vecy",
      status: "active"
    }).returning();
...
- CR\xCDTICO: Responde \xDANICAMENTE con el guion hablado de la nota de voz. NO agregues comentarios, pre\xE1mbulos, explicaciones ni envuelvas el texto en comillas, llaves ({{ }}) o corchetes. Todo tu texto se convertir\xE1 directamente a audio.`;
  ...
  app.post("/admin/trigger-noticia", async (req, res) => {
    const { token, headline, details, isUrgent, targetGroup, force } = req.body;
    if (token !== "vecy2025admin") {
      return res.status(403).json({ error: "Unauthorized" });
    }
    try {
      const { publishNoticiaNacionalNow: publishNoticiaNacionalNow2 } = await Promise.resolve().then(() => (init_cronService(), cronService_exports));
      const result = await publishNoticiaNacionalNow2({
        headline,
        details,
