...
async function geocodeLocation(address) {
  ...
  try {
    ...
    const url = `https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(query + ", Colombia")}&format=json&addressdetails=1&limit=1`;
    
    const response = await fetch(url, {
      headers: { 'User-Agent': 'VecyNetwork/1.0 (eduardo@vecy.network)' }
    });
    ...
    return {
      city: addr.city || addr.town || addr.village || addr.county || 'Bogotá',
      locality: addr.suburb || addr.borough || addr.district || null,
      neighborhood: addr.neighbourhood || addr.residential || addr.quarter || null,
    ...
    console.error(`      ⚠️ Geocoding error for "${address}":`, error.message);
    return null;
  }
}
...
async function extractPropertyData(owner, repo) {
  try {
      ...
      try {
        ...
        if (data.type === 'file') {
          ...
          if (path.endsWith('.json')) {
            ...
            propertyData = parsePropertyFromReadme(content, repo);
            break;
          }
        }
      } catch (e) {
    ...
    // ── [NEW] Geocoding Step ──
    console.log(`   📍 Geocoding: ${propertyData.location}...`);
    const geo = await geocodeLocation(propertyData.location);
    if (geo) {
      console.log(`      ✅ Resolved: ${geo.neighborhood || geo.locality || geo.city} (${geo.coordinates.lat}, ${geo.coordinates.lng})`);
      ...
      try {
        ...
        if (propertyData) {
          syncedProperties.push({
            ...
            addressCity: propertyData.geo?.city || 'Bogotá',
            addressLocality: propertyData.geo?.locality || null,
            addressNeighborhood: propertyData.geo?.neighborhood || null,
            coordinates: propertyData.geo?.coordinates || null,
