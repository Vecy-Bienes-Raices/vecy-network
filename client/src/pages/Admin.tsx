...
function BotStatusWidget() {
  const { data: status, isLoading, isError, refetch } = trpc.janIA.getBotStatus.useQuery(undefined, {
    refetchInterval: 300000, // Refrescar suavemente cada 5 minutos
    staleTime: 120000,
  ...
  return (
    <div className="h-screen bg-background flex flex-col md:flex-row text-foreground relative overflow-hidden font-sans">
      ...
      >
        ...
        {/* Navigation Tabs */}
        <nav className={`flex-1 overflow-y-auto scrollbar-hide space-y-2 ${sidebarExpanded ? 'p-3' : 'p-2 flex flex-col items-center'}`}>
          {tabs.map((tab) => {
            const Icon = tab.icon;
            ...
            return (
              <button
                ...
                className={`
                  transition-all group relative flex items-center
                  ${sidebarExpanded 
                    ? 'w-full gap-3 px-3 py-2.5 rounded-xl text-xs font-semibold' 
                    : 'w-11 h-11 justify-center rounded-xl p-0'
                  }
                  ${isActive 
                    ? 'bg-gradient-to-r from-[#bf953f]/25 via-[#bf953f]/15 to-[#bf953f]/5 text-[#fcf6ba] border border-[#bf953f]/40 shadow-[0_0_15px_rgba(191,149,63,0.15)]' 
                    : 'text-zinc-400 hover:text-zinc-100 hover:bg-white/5 border border-transparent'}
                `}
                title={!sidebarExpanded ? tab.label : undefined}
              >
                <Icon className={`shrink-0 transition-transform group-hover:scale-110 ${sidebarExpanded ? 'w-4 h-4' : 'w-5 h-5'} ${isActive ? 'text-[#bf953f]' : 'text-zinc-400 group-hover:text-zinc-200'}`} />
                ...
                {sidebarExpanded && (
                  <>
                    ...
                    {isActive && (
                      <span className="w-1.5 h-1.5 rounded-full bg-primary shadow-[0_0_8px_#bf953f] shrink-0" />
                    )}
                  </>
                )}
              </button>
        ...
        {/* Footer Links */}
        <div className={`border-t border-white/5 space-y-1.5 shrink-0 bg-black/20 ${sidebarExpanded ? 'p-3' : 'p-2 flex flex-col items-center'}`}>
          <button
            onClick={() => navigate('/')}
            className={`
              transition-all group relative flex items-center
              ${sidebarExpanded 
                ? 'w-full gap-3 px-3 py-2.5 rounded-xl text-xs font-medium text-zinc-400 hover:text-white hover:bg-white/5' 
                : 'w-11 h-11 justify-center rounded-xl p-0 text-zinc-400 hover:text-white hover:bg-white/5'
              }
            `}
            title={!sidebarExpanded ? 'Sitio Público' : undefined}
          >
            <Home className={`shrink-0 ${sidebarExpanded ? 'w-4 h-4' : 'w-5 h-5'}`} />
            {sidebarExpanded && <span className="truncate flex-1 text-left">Sitio Público</span>}
          </button>
          <button
            onClick={logout}
            className={`
              transition-all group relative flex items-center
              ${sidebarExpanded 
                ? 'w-full gap-3 px-3 py-2.5 rounded-xl text-xs font-medium text-red-400/80 hover:text-red-400 hover:bg-red-500/10' 
                : 'w-11 h-11 justify-center rounded-xl p-0 text-red-400/80 hover:text-red-400 hover:bg-red-500/10'
              }
            `}
            title={!sidebarExpanded ? 'Cerrar Sesión' : undefined}
          >
            <LogOut className={`shrink-0 ${sidebarExpanded ? 'w-4 h-4' : 'w-5 h-5'}`} />
            {sidebarExpanded && <span className="truncate flex-1 text-left">Cerrar Sesión</span>}
          </button>
        </div>
      </aside>
