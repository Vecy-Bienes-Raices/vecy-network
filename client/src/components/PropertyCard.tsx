...
export default function PropertyCard({
  ...
  // Formato: "[Tipo] en [Barrio], [Localidad], [Ciudad]"
  const propertyLabel = PROPERTY_TYPE_LABELS[propertyType as PropertyType] || 'Inmueble';
  
  // Use geocoded fields if available, otherwise fallback to name/location
  const displayNeighborhood = neighborhood || zone || null;
  const displayLocality = locality || null;
  const displayCity = city || 'Bogotá';

  const formattedTitle = displayNeighborhood 
    ? `${propertyLabel} en ${displayNeighborhood}${displayLocality ? `, ${displayLocality}` : ''}`
    : name;

  const displayLocation = displayNeighborhood
